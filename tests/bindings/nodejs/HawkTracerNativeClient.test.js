const htcAddon = require('bindings')('hawk_tracer_client');   // loads build/Release/hawk_tracer_client.node
...
describe("2. Set up callback", () => {
    ...
    test("receives all events if onEvents() is called after start()", (done) => {
        ...
        let count = 0;
        hawkTracerClient.start();
        hawkTracerClient.onEvents((events) => {
            count += events.length;
...
describe("3. Start native HawkTracerClient", () => {
    let hawkTracerClient;
    afterEach(() => hawkTracerClient.stop());
...
describe("5. Stop native HawkTracerClient", () => {
    let hawkTracerClient;
    beforeEach(() => {
        const source = require('path').join(__dirname, 'test.htdump');
        // noinspection JSValidateTypes
